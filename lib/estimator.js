// ---------------------------------------------------------------------------
// Configurable pricing engine. Prices come exclusively from data/pricing.json
// (editable by the owner in /admin/pricing) — never from AI, never hardcoded
...
export async function getEstimate({ tipoTrabajo, tamano, condicion, addOns = [], hasPhotos }) {
  const rule = getPricingForService(tipoTrabajo);
  if (!rule) {
    return {
      error: true,
