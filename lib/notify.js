// ---------------------------------------------------------------------------
// Notifications to the business owner (email always; SMS only for
...
//   - SMS:               set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN,
//                        TWILIO_SMS_FROM in .env.local      (https://twilio.com)
...
// The business's real inbox is a Gmail address (maykapainting@gmail.com),
// so until Maykar buys and verifies a domain in Resend, this stays on the
// shared test sender — real delivery to the owner already works via
// `config.notifyEmail`, this only affects the "From" display address.
...
export async function notifyNewLead(lead) {
  ...
  const results = await Promise.allSettled([
    ...
    isHighValue || isConfirmedEmergency ? sendSms(config, summary, lead) : Promise.resolve({ skipped: true }),
...
async function sendSms(config, summary, lead) {
  const sid = process.env.TWILIO_ACCOUNT_SID;
  const token = process.env.TWILIO_AUTH_TOKEN;
  const from = process.env.TWILIO_SMS_FROM;
  if (!sid || !token || !from) {
    console.log("\n[DEMO · SMS NOT sent — Twilio not configured]\n" + summary + "\n");
    return { skipped: false };
  }
