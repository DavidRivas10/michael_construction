...
export function getPricingForService(serviceId) {
  return listPricing().find((p) => p.serviceId === serviceId) || null;
}

export function updatePricingForService(serviceId, partial) {
  const all = listPricing();
  const idx = all.findIndex((p) => p.serviceId === serviceId);
  if (idx === -1) return null;
  all[idx] = { ...all[idx], ...partial };
  writeJSON("pricing.json", all);
  return all[idx];
}
...
export function createLead(data) {
  ...
  const lead = {
    ...
    hasUrgentWord: !!data.hasUrgentWord,
    estado: "nuevo", // nuevo | contactado | agendado | cerrado
    notasInternas: "",
    notified: null, // se completa después de notifyNewLead — { businessEmail, customerEmail, sms, call }
