...
function rowToPortfolio(row) {
  return {
    ...
    hasPhotos: Boolean(row.before_url || row.after_url),
  };
}

// ---- Reseñas ----
// Las reseñas que un admin agrega desde el panel quedan aprobadas al
// instante (isApproved: true). Las que un visitante manda desde el sitio
// público (POST /api/reviews/submit) entran como pendientes (isApproved:
// false) y solo se muestran en el sitio una vez que Michael las aprueba
// desde /admin/testimonios — así se evita spam/abuso sin perder la
// posibilidad de que clientes reales dejen su propia reseña.
export async function listReviews({ onlyApproved = true } = {}) {
  const supabase = getSupabase();
...
export async function addReview(review, { isApproved = true } = {}) {
  const supabase = getSupabase();
  const newReview = {
    id: `r${Date.now()}`,
...
export async function updateReview(id, partial) {
  const supabase = getSupabase();
  const patch = {};
  if (partial.isApproved !== undefined) patch.is_approved = partial.isApproved;
